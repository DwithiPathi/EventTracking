...
const PreviousEvents = () => {
  ...
  const searchEvents = async (title) => {
    console.log(title,"daatsaagjh");
    if (title === "") {
      EventService.getAllPreviousEventsEvents().then((response) => {
        setPreviousEvent(response.data);
        console.log(response.data);
      });
    } else {
      
      const response = await fetch(`${URL}/location1?eventLocation=${title}`);
    ...
    <>
      ...
      <section className="py-4 container">
        <div className="row justify-content-center">
          ...
          {previousevent &&
            previousevent.map((events, index) => {
              return (
                <div className="col-11 col-md-6 col-lg-4 mx-0 mb-4">
                  <div className="card p-0 overflow-hidden h-100 shadow">
                    <div className="card-body">
                      ...
                      <p className="card-text">{events.organizationName}</p>
